...
{
    qDebug() << "----------------------------------------APMSensorsComponentController构造函数" << endl;
    _compassCal.setVehicle(_vehicle);
    qDebug() << "vehicle11111111111111111111111111111  " << _vehicle->id() << endl;
...
//    // Find the sensors component
    foreach (const QVariant& varVehicleComponent, apmPlugin->vehicleComponents()) {
        ...
        qDebug() <<"_sensorsComponent"<< _sensorsComponent << endl;
...
void APMSensorsComponentController::_startVisualCalibration(void)
{
    ...
    _progressBar01->setProperty("value", 0);
    _progressBar02->setProperty("value", 0);
    _progressBar03->setProperty("value", 0);
...
//    _cancelButton->setEnabled(false);
    ...
    if (code == StopCalibrationSuccess) {
        _resetInternalState();
        _progressBar01->setProperty("value", 1);
        _progressBar02->setProperty("value", 1);
        _progressBar03->setProperty("value", 1);
...
void APMSensorsComponentController::_mavCommandResult(int vehicleId, int component, int command, int result, bool noReponseFromVehicle)
{
    ...
    if (command == MAV_CMD_DO_CANCEL_MAG_CAL) {
        ...
        if (result == MAV_RESULT_ACCEPTED) {
            qDebug() << "1111111111111111111111111111111111111111111111111111111111111111" << endl;
...
//            _appendStatusLog(tr("Rotate the vehicle randomly around all axes until the progress bar fills all the way to the right ."));
            ...
            qDebug() << "2222222222222222222222222222222222222222222222222222222222222222222" << endl;
        ...
        qDebug() << "3333333333333333333333333333333333333333333333333333333333333333333" << endl;
...
void APMSensorsComponentController::levelHorizon(void)
{
    ...
    qDebug() << "sssssssssssssssssssssssssssaaaaaaaaaaaaaaaaaaaaaaaa" << endl;
...
void APMSensorsComponentController::_handleUASTextMessage(int uasId, int compId, int severity, QString text)
{
    ...
    if (text.contains(QLatin1Literal("progress <"))) {
        ...
        if (ok && _progressBar01) {
            _progressBar01->setProperty("value", (float)(p / 100.0));
        }
//        if (ok && _progressBar02) {
//            _progressBar01->setProperty("value", (float)(p / 100.0));
//        }
//        if (ok && _progressBar03) {
//            _progressBar01->setProperty("value", (float)(p / 100.0));
//        }
        return;
    }
...
{
    qDebug() << "_calTypeInProgress" << _calTypeInProgress << endl;
    if (_calTypeInProgress == CalTypeLevelHorizon) {
        mavlink_command_ack_t commandAck;
        mavlink_msg_command_ack_decode(&message, &commandAck);

        qDebug() << "APMSensorsComponentController————————_handleCommandAck" << commandAck.command << commandAck.result <<endl;
...
//                _appendStatusLog(tr("Level horizon failed"));
    ...
    if (_calTypeInProgress == CalTypePressure) {
        mavlink_command_ack_t commandAck;
        mavlink_msg_command_ack_decode(&message, &commandAck);
        qDebug() << "commandAck.command:" << commandAck.command  << "2"<< endl;
...
//            _rgCompassCalProgress[magCalProgress.compass_id] = magCalProgress.completion_pct / compassCalCount;
            _rgCompassCalProgress[magCalProgress.compass_id] = magCalProgress.completion_pct;

        }

        qDebug() << _rgCompassCalProgress[0] << _rgCompassCalProgress[1] << _rgCompassCalProgress[2] << endl;

        if (_progressBar01) {
//            _progressBar->setProperty("value", (float)(_rgCompassCalProgress[0] + _rgCompassCalProgress[1] + _rgCompassCalProgress[2]) / 100.0);
            _progressBar01->setProperty("value",(float)(_rgCompassCalProgress[0] / 100.0));
        }

        if (_progressBar02) {
            _progressBar02->setProperty("value",(float)(_rgCompassCalProgress[1] / 100.0));
        }

        if (_progressBar03) {
            _progressBar03->setProperty("value",(float)(_rgCompassCalProgress[2] / 100.0));
        }
    }
}
...
void APMSensorsComponentController::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    case MAVLINK_MSG_ID_COMMAND_ACK:
        qDebug() << "MAVLINK_MSG_ID_COMMAND_ACK" << endl;
        _handleCommandAck(message);
        break;
    case MAVLINK_MSG_ID_MAG_CAL_PROGRESS:
        qDebug() << "MAVLINK_MSG_ID_MAG_CAL_PROGRESS" << endl;
        _handleMagCalProgress(message);
        break;
    case MAVLINK_MSG_ID_MAG_CAL_REPORT:
        qDebug() << "MAVLINK_MSG_ID_MAG_CAL_REPORT" << endl;
