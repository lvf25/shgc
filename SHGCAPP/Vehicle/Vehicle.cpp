...
//    connect(_uas, &UAS::imageReady,                     this, &Vehicle::_imageReady);
    ...
    // connect this vehicle to the follow me handle manager连接手柄
...
void Vehicle::_handleSysStatus(mavlink_message_t& message)
{
    ...
    _droprateFact.setRawValue(100 - sysStatus.drop_rate_comm);
...
//            _say(QString(tr("%1 低电量: %2 剩余电量")).arg(_vehicleIdSpeech()).arg(sysStatus.battery_remaining));
...
//    qDebug() << "_battery1FactGroup.current():" << _battery1FactGroup.current()->rawValue().toFloat();
...
{
    qDebug() << "setFlightMode" << flightMode << endl;
    uint8_t     base_mode;
...
{
    qDebug() << "_handleFlightModeChanged" << flightMode << endl;
...
void Vehicle::_sendMavCommandAgain(void)
{
    ...
    if (queuedCommand.commandInt) {
        qDebug() << "mavlink_command_int_t..." << endl;
        ...
        mavlink_msg_command_long_encode_chan(_mavlink->getSystemId(),
                                             ...
                                             &msg,
                                             &cmd);
        qDebug() << "mavlink_command_long_t..." << cmd.command <<endl;
    }
...
void Vehicle::_handleCommandAck(mavlink_message_t& message)
{
    ...
    qDebug() << "vehicle————————_handleCommandAck" << ack.command << ack.result <<endl;
...
{
    QGeoCoordinate gcsPosition = _toolbox->qgcPositionManager()->gcsPosition();  //获取地面站坐标
