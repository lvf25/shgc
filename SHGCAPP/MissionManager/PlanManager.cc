...
void PlanManager::_handleMissionRequest(const mavlink_message_t& message, bool missionItemInt)
{
    ...
    qDebug() << "----------------------missionRequest.seq:" << missionRequest.seq << endl;
    ...
    qDebug() << item->param1() << item->param2() << item->param3() << item->param4() << item->param5() << item->param6() << endl;

    
    if(_vehicle->sendMessageOnLink(_dedicatedLink, messageOut))
    {
        emit sendtovehicle(missionRequest.seq);
    }
    _startAckTimeout(AckMissionRequest);
}
...
void PlanManager::_finishTransaction(bool success, bool apmGuidedItemWrite)
{
    ...
    qDebug() << "TransactionType_t" << (TransactionType_t)currentTransactionType << endl;
